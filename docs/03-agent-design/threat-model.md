...
| Answer-key leakage | Loader reads only allowed manifest entries under evidence root | Model-input spy test | `ERROR` |
...
| Edit bypasses verification | Edit always creates a new digest and review round | Approval-after-edit test | `REVISION_REQUESTED` |
